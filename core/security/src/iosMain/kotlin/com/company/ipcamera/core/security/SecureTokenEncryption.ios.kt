...
/**
 * iOS реализация обфускации токенов.
 *
 * NB: В офлайн-окружении нет KMP-криптопровайдера (SecKey-кори has no AES-GCM interop here).
 * На время используется детерминированная XOR-обфускация с тем же ключом, что и на Native
 * (reversible, consistent между платформами) — защита от открытого хранения/логов,
 * НЕ полноценное шифрование. Для production перейти на KMP AES (напр. Krypto-lang).
 */
actual class SecureTokenEncryption : TokenEncryption {
    ...
    companion object {
        // Тот же детерминированный ключ, что и в SecureTokenEncryption.native.kt.
