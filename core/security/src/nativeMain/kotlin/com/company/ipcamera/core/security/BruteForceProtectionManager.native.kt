...
/**
 * Native реализация защиты от брутфорс атак (in-memory; Linux/Windows/macOS).
 *
 * Логика идентична JVM-реализации (см. BruteForceProtectionManager.jvm.kt):
 * скользящее окно неудачных попыток + блокировка при достижении порога из [BruteForceConfig].
 * Kotlin/Native: new-memory model — мутации под одним lock-объектом.
 */
actual class BruteForceProtectionManager : BruteForceProtection {
    ...
    actual constructor(config: BruteForceConfig) : this(config, { kotlin.system.getTimeMillis() })
    ...
    private val userAttempts = mutableMapOf<String, MutableList<Long>>()
    private val ipAttempts = mutableMapOf<String, MutableList<Long>>()
    private val userBlockedUntil = mutableMapOf<String, Long>()
    private val ipBlockedUntil = mutableMapOf<String, Long>()
