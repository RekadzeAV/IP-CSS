...
actual class NativeRtspClient {
    ...
    actual fun setFrameCallback(
        ...
        val cCallback: CPointer<CFunction<(CPointer<RTSPFrame>?, COpaquePointer?) -> Unit>> =
    ...
    actual fun setReconnectParams(
        ...
        memScoped {
            ...
            params.enabled = enabled.convert()
            params.maxRetries = maxRetries.convert()
            params.initialDelayMs = initialDelayMs.convert()
            params.maxDelayMs = maxDelayMs.convert()
            params.backoffMultiplier = backoffMultiplier.convert()
    ...
    private fun handleToPointer(handle: Long): CPointer<RTSPClient>? {
        return if (handle == 0L) {
            ...
            interpretCPointer<RTSPClient>(handle)
        }
    }
    ...
    private fun convertNativeFrame(framePtr: CPointer<RTSPFrame>?): RtspFrame {
        if (framePtr == null) {
            return RtspFrame(
                data = ByteArray(0),
                timestamp = 0L,
        ...
        if (dataPtr != null && size > 0) {
            // Копируем данные из нативного буфера напрямую
            for (i in 0 until size) {
                data[i] = dataPtr[i].toByte()
            }
        }
