...
class OnvifClientTest {
    ...
    @Test
    fun testOnvifCapabilitiesWithAllServices() {
        val capabilities = OnvifCapabilities(
            deviceServiceUrl = "http://192.168.1.100/onvif/device_service",
            mediaServiceUrl = "http://192.168.1.100/onvif/media_service",
            ptzServiceUrl = "http://192.168.1.100/onvif/ptz_service",
            eventServiceUrl = "http://192.168.1.100/onvif/event_service",
            analyticsServiceUrl = "http://192.168.1.100/onvif/analytics_service",
            imagingServiceUrl = "http://192.168.1.100/onvif/imaging_service"
        )

        assertNotNull(capabilities.deviceServiceUrl)
        assertNotNull(capabilities.mediaServiceUrl)
        assertNotNull(capabilities.ptzServiceUrl)
        assertNotNull(capabilities.eventServiceUrl)
        assertNotNull(capabilities.analyticsServiceUrl)
        assertNotNull(capabilities.imagingServiceUrl)
    }

    @Test
    fun testNormalizeUrl() {
        // Тест нормализации URL будет через рефлексию или публичный метод
        // Пока проверяем, что структура правильная
        assertTrue(true)
    }
}
