...
if ($ShowHelp) {
    Write-Host "Create develop/<platform> and test/<platform> branches from main (git repo required)."
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\scripts\create-platform-branches.ps1 -ShowHelp"
    Write-Host "  .\scripts\create-platform-branches.ps1"
    Write-Host ""
    Write-Host "Exit codes: 0 on success; non-zero if git checkout/branch fails."
    exit 0
}
