...
if ($ShowHelp) {
    Write-Host "Create, update, or merge markdown docs with Alfa-x.y.z style metadata."
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\scripts\manage-documentation.ps1 -ShowHelp"
    Write-Host "  .\scripts\manage-documentation.ps1 -Action create|update|merge [-Document <path>] [-SourceDocuments ...] [-OutputDocument <path>] [-TargetDirectory <dir>] [-Date yyyy-MM-dd]"
    Write-Host ""
    Write-Host "Exit codes: 0 success; 1 validation errors (see script messages)"
    exit 0
}
