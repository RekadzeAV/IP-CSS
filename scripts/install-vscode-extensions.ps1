...
if ($ShowHelp) {
    Write-Host "Install recommended VS Code / Cursor extensions (uses 'cursor' or 'code' on PATH)."
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\scripts\install-vscode-extensions.ps1 -ShowHelp"
    Write-Host "  .\scripts\install-vscode-extensions.ps1"
    Write-Host ""
    Write-Host "Exit codes:"
    Write-Host "  0  Finished (some extensions may fail; see summary)"
    Write-Host "  1  Neither code nor cursor found on PATH"
    exit 0
}
