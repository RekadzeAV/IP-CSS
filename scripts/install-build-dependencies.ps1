...
# File is UTF-8 with BOM so Windows PowerShell parses Cyrillic strings below correctly.
...
if ($ShowHelp) {
    Write-Host "Install/check build stack for IP-CSS on Windows (Chocolatey, Java, Node, CMake, FFmpeg, compilers, Android hints)"
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\scripts\install-build-dependencies.ps1 -ShowHelp"
    Write-Host "  .\scripts\install-build-dependencies.ps1"
    Write-Host ""
    Write-Host "Notes:"
    Write-Host "  Run from repository root (uses gradlew.bat). Administrator recommended for choco installs."
    Write-Host "  Script may prompt (e.g. MinGW install). Review on-screen [ERROR]/[WARN] lines."
    Write-Host ""
    Write-Host "Exit codes:"
    Write-Host "  0  Finished (warnings may still be printed)"
    Write-Host "  1  Chocolatey install failed when admin was required"
    exit 0
}
