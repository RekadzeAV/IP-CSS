...
if ($ShowHelp) {
    Write-Host "Install FFmpeg on Windows (Chocolatey, Scoop, or bundled zip to -InstallPath)."
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\scripts\install-ffmpeg.ps1 -ShowHelp"
    Write-Host "  .\scripts\install-ffmpeg.ps1 [-UseChocolatey|-UseScoop|-Manual] [-InstallPath <dir>]"
    Write-Host ""
    Write-Host "Exit codes:"
    Write-Host "  0  Success or FFmpeg already usable"
    Write-Host "  1  Install failed"
    exit 0
}
