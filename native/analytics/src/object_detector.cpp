...
ObjectDetector* object_detector_create(const ObjectDetectorParams* params) {
    ...
    if (params) {
        ...
        if (detector->params.inputWidth <= 0) detector->params.inputWidth = 640;
        if (detector->params.inputHeight <= 0) detector->params.inputHeight = 640;
    } else {
        detector->params.confidenceThreshold = 0.5f;
        detector->params.maxObjects = 10;
        detector->params.useGPU = false;
        detector->params.inputWidth = 640;
        detector->params.inputHeight = 640;
    }
...
bool object_detector_detect(
    ...
    try {
        ...
        // Подготовка входного блоба для DNN (размер из params: 416/640 для YOLO)
        int inW = detector->params.inputWidth > 0 ? detector->params.inputWidth : 640;
        int inH = detector->params.inputHeight > 0 ? detector->params.inputHeight : 640;
        cv::Mat blob;
        cv::dnn::blobFromImage(frame, blob, 1.0/255.0, cv::Size(inW, inH), cv::Scalar(0, 0, 0), true, false);
        ...
        // TODO: Реализовать полный парсинг выходов YOLO модели
        // Требуется обработка outputs для извлечения bounding boxes, классов и уверенности
        // Применение Non-Maximum Suppression (NMS) для фильтрации дубликатов
        // Формат YOLO: [x_center, y_center, width, height, confidence, class_scores...]
