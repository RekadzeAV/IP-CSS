...
if ($ShowHelp) {
    Write-Host "Create native/*/lib/windows/x64 directories as placeholders for cinterop integration smoke."
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\native\build-stub-libs.ps1 -ShowHelp"
    Write-Host "  .\native\build-stub-libs.ps1"
    Write-Host ""
    Write-Host "Exit codes: 0 after creating directories (no real DLLs; see script notes)."
    exit 0
}
