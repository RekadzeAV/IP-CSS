...
export const fetchCameras = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to fetch cameras'));
    }
  }
);
...
export const fetchCameraById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to fetch camera'));
    }
  }
);
...
export const createCamera = createAsyncThunk(
  ...
  async (data: CreateCameraRequest, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to create camera'));
    }
  }
);
...
export const updateCamera = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: UpdateCameraRequest }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to update camera'));
    }
  }
);
...
export const deleteCamera = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to delete camera'));
    }
  }
);
...
export const testConnection = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to test connection'));
    }
  }
);
...
export const discoverCameras = createAsyncThunk(
  ...
  async (protocol: 'onvif' | 'rtsp' | 'all' = 'onvif', { rejectWithValue }) => {
    try {
      const cameras = await cameraService.discoverCamerasWithProtocol(protocol);
      return cameras;
    } catch (error: unknown) {
      return rejectWithValue(getErrorMessage(error, 'Failed to discover cameras'));
    }
  }
);
...
const camerasSlice = createSlice({
  ...
  reducers: {
    ...
    updateCameraStatus: (state, action: PayloadAction<{ cameraId: string; status: string }>) => {
      ...
      if (camera) {
        camera.status = action.payload.status as CameraStatus;
      }
      if (state.selectedCamera?.id === action.payload.cameraId) {
        state.selectedCamera.status = action.payload.status as CameraStatus;
      }
    },
