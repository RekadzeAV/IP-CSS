...
export const fetchEventById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to fetch event'));
    }
  }
);
...
export const acknowledgeEvent = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to acknowledge event'));
    }
  }
);
...
export const acknowledgeEvents = createAsyncThunk(
  ...
  async (ids: string[], { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to acknowledge events'));
    }
  }
);
...
export const fetchStatistics = createAsyncThunk(
  ...
  async (params: { cameraId?: string; startTime?: number; endTime?: number }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(getErrorMessage(error, 'Failed to fetch statistics'));
    }
  }
);
...
const eventsSlice = createSlice({
  ...
  reducers: {
    ...
    addEventFromWebSocket: (state, action: PayloadAction<Event>) => {
      ...
      if (existingIndex !== -1) {
        // Reconnect/replay safety: update existing item instead of duplicating it.
    ...
    // WebSocket: частичное обновление (например event.acknowledged)
    updateEventPartiallyFromWebSocket: (
      state,
      action: PayloadAction<Partial<Event> & { id: string }>
    ) => {
      const index = state.events.findIndex((e) => e.id === action.payload.id);
      if (index !== -1) {
    ...
    // WebSocket: удаление события (event.deleted)
    removeEventFromWebSocket: (state, action: PayloadAction<string>) => {
