...
const schema: yup.ObjectSchema<CameraEditFormData> = yup.object({
...
export default function CameraEditDialog({ open, onClose, camera }: CameraEditDialogProps) {
  ...
  const onSubmit = async (data: CameraEditFormData) => {
    if (!camera) return;

    try {
      setLoading(true);
      await dispatch(updateCamera({ id: camera.id, data: data as UpdateCameraRequest })).unwrap();
      enqueueSnackbar('Камера успешно обновлена', { variant: 'success' });
      onClose();
    } catch (error: unknown) {
      enqueueSnackbar(getErrorMessage(error, 'Ошибка при обновлении камеры'), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
