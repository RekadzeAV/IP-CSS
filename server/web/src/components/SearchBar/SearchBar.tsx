...
export default function SearchBar({
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      // Поиск событий
      try {
        const eventsResponse = await eventService.getEvents({ limit: 50 });
        const eventMatches = eventsResponse.items.filter(
          (event) =>
      ...
      // Поиск записей
      try {
        const recordingsResponse = await recordingService.getRecordings({ limit: 50 });
        const recordingMatches = recordingsResponse.items.filter(
          (recording) =>
