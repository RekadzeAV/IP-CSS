...
export default function PTZControls({ camera, disabled = false }: PTZControlsProps) {
  ...
  }, [camera.id, supportsPTZ, loadPresets]);
  ...
  const handlePTZAction = useCallback(
    async (action: 'move' | 'zoom', direction: string) => {
      ...
      try {
        ...
        enqueueSnackbar(getErrorMessage(error, 'Ошибка управления PTZ'), { variant: 'error' });
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleStop = useCallback(async () => {
    ...
    try {
      ...
      enqueueSnackbar(getErrorMessage(error, 'Ошибка остановки PTZ'), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  ...
  const handleSavePreset = async () => {
    ...
    try {
      ...
      enqueueSnackbar(getErrorMessage(error, 'Ошибка сохранения пресета'), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRecallPreset = async (presetId: number) => {
    try {
      ...
      enqueueSnackbar(getErrorMessage(error, 'Ошибка вызова пресета'), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeletePreset = async (presetId: number) => {
    ...
    try {
      ...
      enqueueSnackbar(getErrorMessage(error, 'Ошибка удаления пресета'), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
