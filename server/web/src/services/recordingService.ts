...
export const recordingService = {
   ...
   */
  /**
   * Абсолютный URL HLS playlist для записи (совпадает с `NEXT_PUBLIC_API_URL`; cookie-сессия как у остального API).
   */
  getHlsPlaylistUrl(
    recordingId: string,
    quality: 'low' | 'medium' | 'high' | 'ultra' = 'medium'
  ): string {
    return `${API_URL}/recordings/${recordingId}/hls/playlist.m3u8?quality=${quality}`;
  },
