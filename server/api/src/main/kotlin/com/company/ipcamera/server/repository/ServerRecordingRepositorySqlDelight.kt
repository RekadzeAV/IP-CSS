...
/**
 * Серверная реализация RecordingRepository с использованием SQLDelight / PostgreSQL.
...
class ServerRecordingRepositorySqlDelight(
    ...
    override suspend fun addRecording(recording: Recording): Result<Recording> = withContext(Dispatchers.IO) {
        try {
            // Явная проверка дубликата: INSERT OR REPLACE в .sq молча перезаписал бы запись
            val existing = database.cameraDatabaseQueries.selectRecordingById(recording.id)
                .executeAsOneOrNull()
            if (existing != null) {
                return@withContext Result.failure(
                    IllegalArgumentException("Recording already exists: ${recording.id}")
                )
            }
    ...
    override suspend fun getDownloadUrl(id: String): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            // Возвращаем базовый URL для скачивания
            // Подписанные URL генерируются на уровне маршрутов (RecordingRoutes) через SignedUrlService
            // Это обеспечивает безопасность и контроль доступа
