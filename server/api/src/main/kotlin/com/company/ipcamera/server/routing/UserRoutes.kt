                ...
                put {
                    ...
                    try {
                        ...
                        result.fold(
                            onSuccess = { user ->
                                val principal = call.principal<io.ktor.server.auth.jwt.JWTPrincipal>()
                                val updatedBy = principal?.payload?.subject ?: "unknown"

                                // Логируем изменение роли, если она изменилась
                                if (existingUser.role != user.role) {
                                    SecurityLogger.log(
                                        com.company.ipcamera.server.security.SecurityEvent(
                                            ...
                                            details = mapOf(
                                                "action" to "role_change",
                                                "target_user_id" to user.id,
                                                "target_username" to user.username,
                                                "old_role" to existingUser.role.name,
                                                "new_role" to user.role.name
                                            )
                                        )
                                    )
                                }
