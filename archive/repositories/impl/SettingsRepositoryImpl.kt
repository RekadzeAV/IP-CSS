...
class SettingsRepositoryImpl(
    ...
    override suspend fun updateSystemSettings(settings: SystemSettings): Result<Unit> =
        withContext(Dispatchers.Default) {
            try {
                settingsApiService.updateSystemSettings(settings.toDto())
                Result.success(Unit)
            } catch (e: Exception) {
    ...
    override suspend fun resetSettings(category: SettingsCategory?): Result<Unit> =
        withContext(Dispatchers.Default) {
            try {
                settingsApiService.resetSettings(category?.name?.lowercase())
                Result.success(Unit)
            } catch (e: Exception) {
            ...
            recording =
                recording?.let {
                    com.company.ipcamera.shared.domain.model.RecordingSystemSettings(
                        defaultQuality = Quality.valueOf(it.defaultQuality.uppercase()),
                        defaultFormat = RecordingFormat.valueOf(it.defaultFormat.uppercase()),
            ...
            recording =
                recording?.let {
                    RecordingSystemSettings(
                        ...
                        defaultFormat = it.defaultFormat.name,
                        maxDuration = it.maxDuration,
                        autoDelete = it.autoDelete,
                        retentionDays = it.retentionDays,
                    )
