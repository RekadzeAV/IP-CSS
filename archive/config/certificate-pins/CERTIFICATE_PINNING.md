...
- ✅ Android реализация (интеграция доступна, требует регулярной валидации в CI/runtime)
- ✅ iOS реализация (интеграция доступна, требует регулярной валидации в CI/runtime)

## Production baseline

1. `enablePinning=true` для production профилей.
2. На каждый хост минимум 2 pins (active + backup).
3. Изменение pins проходит через PR с проверкой CI профиля.
4. Любой `enforcePinning=false` допускается только для локального/dev профиля.
5. HTTPS-only endpoints для production окружений.

## Дальнейшие улучшения

1. Усилить integration/runtime тесты pinning для реальных окружений
2. Добавить автоматическую проверку backup pins и срока ротации
3. Добавление поддержки обновления pins через remote configuration
4. Ввести обязательный audit trail для изменения pins
