...
class CameraEntityMapperTest {
    private val mapper = CameraEntityMapper(TestPasswordEncryption())
    ...
    fun `test toDomain basic camera`() {
        ...
        val encryptedDbCamera = mapper.toDatabase(sourceCamera)
        val dbCamera =
            com.company.ipcamera.shared.database.Camera(
                id = "camera-1",
                name = "Test Camera",
                url = "rtsp://192.168.1.100:554/stream",
                username = "admin",
                password = encryptedDbCamera.password,
                model = "Test Model",
                status = "ONLINE",
    ...
    fun `test toDatabase basic camera`() {
        ...
        assertNotNull(dbCamera.password)
        assertTrue(dbCamera.password!!.startsWith("ENC:"))
    }

    @Test
    fun `test toDomain rejects plaintext password from database`() {
        val strictMapper = CameraEntityMapper(TestPasswordEncryption(), allowPlaintextPasswords = false)
        val dbCamera =
            com.company.ipcamera.shared.database.Camera(
                id = "camera-plaintext",
                name = "Plain Camera",
                url = "rtsp://192.168.1.110:554/stream",
                username = "admin",
        ...
        assertFailsWith<SecurityException> {
            strictMapper.toDomain(dbCamera)
        }
    }
