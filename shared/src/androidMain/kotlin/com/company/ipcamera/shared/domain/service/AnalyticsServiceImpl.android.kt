...
import com.company.ipcamera.shared.domain.model.FaceLandmark
...
/**
 * Android реализация AnalyticsService с использованием NativeAnalytics через JNI
 */
actual class AnalyticsServiceImpl actual constructor() : AnalyticsService {
    ...
    private val objectDetectorModelPathLoaded = mutableMapOf<String, String>() // cameraId -> загруженный путь модели
                        ...
                        .map { face ->
                            val landmarks =
                                if (includeLandmarks && face.landmarks != null) {
                                    val landmarkValues = face.landmarks?.toList().orEmpty()
                                    landmarkValues.chunked(2).mapIndexed { index, coords ->
