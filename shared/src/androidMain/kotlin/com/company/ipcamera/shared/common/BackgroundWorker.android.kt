...
actual class BackgroundWorker actual constructor(private val context: Any?) {
    ...
    private val taskEventsState = MutableStateFlow<TaskEvent?>(null)
    val taskEventsFlow: Flow<TaskEvent>
        get() = taskEventsState.asStateFlow().filterNotNull()
    ...
    actual suspend fun getTaskStatus(taskId: String): TaskStatus? {
        return withContext(Dispatchers.IO) {
            try {
                val workInfos = workManager.getWorkInfosByTag(taskId).get()
                val workInfo = workInfos.firstOrNull() ?: return@withContext null
