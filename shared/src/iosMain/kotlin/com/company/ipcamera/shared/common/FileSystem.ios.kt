...
actual class FileSystem actual constructor(context: Any?) {
    ...
    actual fun getDocumentsDirectory(): String = documentsDirectory

    actual fun getCacheDirectory(): String = cacheDirectory

    actual fun getTempDirectory(): String = tempDirectory
    ...
    actual suspend fun createDirectory(path: String): Boolean =
        withContext(Dispatchers.Default) {
            try {
                val allowedDirs = listOf(documentsDirectory, cacheDirectory, tempDirectory)
                val validatedPath =
                    PathSecurity.validateAndNormalizePath(path, documentsDirectory)
                        ?: return@withContext false

                if (!PathSecurity.isPathWithinAllowedDirectories(validatedPath, allowedDirs)) {
                    return@withContext false
                }

                val url = NSURL.fileURLWithPath(validatedPath)
    ...
    actual suspend fun ensureDirectory(path: String): Boolean =
        withContext(Dispatchers.Default) {
    ...
    actual suspend fun readTextFile(path: String): String? =
        withContext(Dispatchers.Default) {
            try {
                val allowedDirs = listOf(documentsDirectory, cacheDirectory, tempDirectory)
                val validatedPath =
                ...
                val data = NSData.dataWithContentsOfFile(validatedPath)
                if (data != null) {
        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                if (data != null) {
                    ...
                    memcpy(bytes, data.bytes, data.length.toInt())
                    bytes
                } else {
                    null
                }
    ...
    actual suspend fun writeTextFile(
        path: String,
        text: String,
        append: Boolean,
    ): Boolean =
        withContext(Dispatchers.Default) {
            try {
                ...
                if (data != null) {
                    if (append && exists(validatedPath)) {
                        val existingData = NSData.dataWithContentsOfFile(validatedPath)
                        if (existingData != null) {
                            val mutableData = NSMutableData.dataWithData(existingData)
                            mutableData.appendData(data)
                            mutableData.writeToFile(validatedPath, atomically = true)
                        } else {
                            data.writeToFile(validatedPath, atomically = true)
                        }
                    } else {
                        data.writeToFile(validatedPath, atomically = true)
                    }
            ...
            try {
                ...
                val success =
                    if (recursive) {
                        ...
                        fileManager.removeItemAtPath(validatedPath, error = errorPtr.ptr)
                    }
                success
            } catch (e: Exception) {
                false
    ...
    actual suspend fun listDirectory(path: String): List<String> =
        withContext(Dispatchers.Default) {
            try {
                ...
                val contents = fileManager.contentsOfDirectoryAtPath(validatedPath, error = null)
                if (contents != null) {
                    val basePath = if (validatedPath.endsWith("/")) validatedPath else "$validatedPath/"
    ...
    actual suspend fun getFileSize(path: String): Long? =
        withContext(Dispatchers.Default) {
            try {
                ...
                val attributes = fileManager.attributesOfItemAtPath(validatedPath, error = null)
                if (attributes != null) {
    ...
    actual fun getAbsolutePath(path: String): String = NSURL.fileURLWithPath(path).standardizedURL?.path ?: path

    actual fun getFileName(path: String): String = NSURL.fileURLWithPath(path).lastPathComponent ?: ""
