...
import com.company.ipcamera.shared.domain.model.LandmarkType
import com.company.ipcamera.shared.domain.model.RecognizedLicensePlate
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import com.company.ipcamera.core.network.analytics.ObjectType as NativeObjectType

/**
 * JVM реализация AnalyticsService с использованием NativeAnalytics через JNI
 */
actual class AnalyticsServiceImpl : AnalyticsService {
    private val nativeAnalytics = NativeAnalytics()

    // Кэш handles для каждой камеры
    private val motionDetectorHandles = mutableMapOf<String, Long>()
    private val objectDetectorHandles = mutableMapOf<String, Long>()
    private val objectDetectorModelPathLoaded = mutableMapOf<String, String>() // cameraId -> loaded model path
    ...
    actual override suspend fun detectMotion(
        camera: Camera,
        frameData: ByteArray,
    ...
    actual override suspend fun detectObjects(
        camera: Camera,
        frameData: ByteArray,
        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                if (modelPath != null && objectDetectorModelPathLoaded[camera.id] != modelPath) {
                    ...
                    if (!modelLoaded) {
                        println("Warning: Failed to load object detection model from: $modelPath")
                        return@withContext ObjectDetectionResult(objects = emptyList())
                    }
    ...
    actual override suspend fun trackObjects(
        camera: Camera,
        frameData: ByteArray,
    ): List<TrackInfo> =
        withContext(Dispatchers.Default) {
            try {
                val settings = camera.settings.analytics
                if (!settings.objectDetection) return@withContext emptyList<TrackInfo>()
                ...
                val modelPath = camera.settings.analytics.objectDetectionModelPath
                if (modelPath != null && objectDetectorModelPathLoaded[camera.id] != modelPath) {
                    val loaded = nativeAnalytics.loadObjectDetectorModel(detectorHandle, modelPath)
                    if (!loaded) return@withContext emptyList()
                    objectDetectorModelPathLoaded[camera.id] = modelPath
                }

                val trackerHandle =
                    objectTrackerHandles.getOrPut(camera.id) {
                        nativeAnalytics.createObjectTracker(
                            iouThreshold = 0.5f,
                            maxAge = 30,
                            minConfidence = settings.objectDetectionConfidenceThreshold,
                        ) ?: return@withContext emptyList()
                    }
                ...
                val height = camera.resolution?.height ?: 1080
                val detectResult =
                    nativeAnalytics.detectObjects(detectorHandle, frameData, width, height)
                        ?: return@withContext emptyList()

                val timestampMs = System.currentTimeMillis()
                val trackingResult =
                    nativeAnalytics.updateTracking(trackerHandle, detectResult, timestampMs)
                        ?: return@withContext emptyList()

                trackingResult.objects.map { obj ->
                    TrackInfo(
                        trackId = obj.id,
                        objectType = mapObjectType(obj.type),
                        boundingBox = BoundingBox(obj.x, obj.y, obj.width, obj.height),
                        confidence = obj.confidence,
                        ...
                        .map { face ->
                            val landmarks =
                                if (includeLandmarks && face.landmarks != null) {
                                    face.landmarks!!.toList().chunked(2).mapIndexed { index: Int, coords: List<Int> ->
                                        val lx = coords.elementAtOrNull(0) ?: 0
                                        val ly = coords.elementAtOrNull(1) ?: 0
                                        val type =
                                            when (index) {
    ...
    actual override suspend fun recognizeLicensePlates(
        camera: Camera,
        frameData: ByteArray,
        minConfidence: Float,
        country: String?,
    ...
    private fun mapObjectType(type: NativeObjectType): String {
        return when (type) {
            NativeObjectType.PERSON -> "person"
            NativeObjectType.VEHICLE -> "vehicle"
            NativeObjectType.BICYCLE -> "bicycle"
            NativeObjectType.MOTORCYCLE -> "motorcycle"
            NativeObjectType.UNKNOWN -> "unknown"
        }
    }
    ...
    actual override suspend fun compareFaces(
