...
class EventLocalDataSourceImpl(
    ...
    override suspend fun updateEvent(event: Event): Result<Event> =
        withContext(Dispatchers.Default) {
            // INSERT OR REPLACE (см. insertEvent в .sq): чистый UPDATE не вставляет строку, если её ещё нет.
            upsertEvent(event)
        }

    private suspend fun upsertEvent(event: Event): Result<Event> {
        return try {
            val dbEvent = mapper.toDatabase(event)
            database.cameraDatabaseQueries.insertEvent(
                id = dbEvent.id,
            ...
            logger.error(e) { "Error upserting event in local database: ${event.id}" }
            Result.failure(e)
        }
    }
