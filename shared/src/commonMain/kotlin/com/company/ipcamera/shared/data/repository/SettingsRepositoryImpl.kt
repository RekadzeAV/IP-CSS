    ...
    override suspend fun getSettings(category: SettingsCategory?): List<Settings> =
        withContext(Dispatchers.Default) {
            try {
                ...
                // Local-first: remote только если локально пусто
                if (remoteDataSource != null && localSettings.isEmpty()) {
            ...
            try {
                ...
                val localResult =
                    if (existingSetting != null) {
                        localDataSource.updateSetting(existingSetting.copy(value = value, updatedAt = now))
                    } else {
                        val category = categoryFromDottedKey(key)
                        localDataSource.saveSetting(
                            Settings(
            ...
            try {
                ...
                remoteDataSource?.getSystemSettings()?.fold(
                    onSuccess = { systemSettings ->
                        // SystemSettings обычно не кэшируются локально, так как они системные
                        // и должны всегда получаться с сервера для актуальности
                        systemSettings
                    },
                    onError = {
