        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                val currentUser: User? =
                    localUsers.firstOrNull { it.isActive } ?: run {
                        if (userApiService != null) {
                            try {
                                val response = userApiService.getCurrentUser()
                                val u = response.toDomain()
                                localDataSource.saveUser(u)
                                u
                            } catch (e: Exception) {
                            ...
                            val apiResult = remoteDataSource?.getCurrentUser()
                            if (apiResult != null) {
                                apiResult.fold(
                                    onSuccess = { u ->
                                        localDataSource.saveUser(u)
                ...
                if (userApiService != null) {
                    ...
                    remoteDataSource.updateUser(user.id, user).fold(
                        ...
                        onError = { error ->
                            logger.warn(
                                error,
                            ) { "Failed to sync user update to remote, but updated locally: ${user.id}" }
                            localResult
                        },
                    )
                } else {
    ...
    override suspend fun getUsers(
        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                // Local-first: remote только если локально нет пользователей (с учётом role)
                if (remoteDataSource != null && filteredUsers.isEmpty()) {
