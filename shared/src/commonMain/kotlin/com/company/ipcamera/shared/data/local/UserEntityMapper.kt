...
internal class UserEntityMapper {
    ...
    private fun parseUserRole(raw: String): UserRole {
        val key = raw.trim()
        return enumValues<UserRole>().firstOrNull { it.name.equals(key, ignoreCase = true) }
            ?: runCatching { UserRole.valueOf(key.uppercase()) }.getOrElse { UserRole.VIEWER }
    }
}
