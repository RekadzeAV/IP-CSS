...
class UserLocalDataSourceImpl(
    ...
    override suspend fun updateUser(user: User): Result<User> =
        withContext(Dispatchers.Default) {
            // INSERT OR REPLACE (см. insertUser в .sq): чистый UPDATE не вставляет строку, если её ещё нет —
            // тогда локальное сохранение «молча» не происходило при update без предшествующего save.
            upsertUser(user)
        }

    private suspend fun upsertUser(user: User): Result<User> {
        return try {
            val dbUser = mapper.toDatabase(user)
            database.cameraDatabaseQueries.insertUser(
                id = dbUser.id,
            ...
            logger.error(e) { "Error upserting user in local database: ${user.id}" }
            Result.failure(e)
        }
    }
